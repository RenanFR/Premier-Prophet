...
def fetch_fixture_data():
    ...
    headers = {'x-rapidapi-key': 'd8aac0e59dmsh903ef91b3450d32p14e614jsn6d25591ff31d'}
    
    # Check if the data is already saved
    if os.path.exists('fixture_data.json'):
        with open('fixture_data.json', 'r') as file:
            data = json.load(file)  # Use json.load to load the data as a dictionary
    else:
...
def get_fixtures():
    ...
    fixtures_json = df.to_json(orient='records', date_format='iso', default_handler=str)
